...
async function createPaymentProposal(governanceAddress: string, description: string): Promise<bigint> {
  ...
  const proposaltx = await governance.createPaymentProposal(token, amount, to, description);
  await proposaltx.wait();
  const proposalId = await getProposalId(governance, proposaltx.blockNumber as number) // ProposalCreated
